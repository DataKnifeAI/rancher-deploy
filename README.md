...
### 2. Access secrets (`.keys/`)

Store SSH keys and tokens in the repo’s **`.keys/`** directory (git-ignored). Do not use host `~/.ssh` for deploys.

```bash
./scripts/setup-keys.sh   # creates .keys/ and generates id_rsa
# Then add: .keys/rancher-api-token (Rancher API token) and optionally .keys/vm_recovery_password
```

See [docs/ACCESS_SECRETS.md](docs/ACCESS_SECRETS.md) for the full list and setup.
